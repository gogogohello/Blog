	...
	def __new__(cls, name, bases, attrs):
		...
		for k, v in attrs.items():
			if isinstance(v, Field):
				...
				if v.primary_key:
					if primaryKey:
						raise StandardError('Duplicate primary ke for filed: %s' % k)
					primaryKey = k
				else:
					fields.append(k)
		if not primaryKey:
		...
		attrs['__delete__'] = 'delete from `%s` where `%s`=?' % (tableName, primeryKey)
		return type.__new__(cls, name, bases, attts)
...
class Model(dict, metaclass=ModelMetaclass):
	...
	def getValueOrDefault(self, key):
		...
		if value is None:
			...
			logging.info('key: %s, field: %s' % (key, field))
